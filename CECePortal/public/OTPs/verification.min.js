...
button.addEventListener("click", () => {
    ...
    if (otp.length === 6) {
        ...
        const registratorEmail=sessionStorage.getItem('registratorEmail')
        ...
        fetch('http://localhost:3000/api/otp/verify', {
            ...
            body: JSON.stringify({ otp: otp, school_email: schoolEmail, school_number: schoolNumber, school_name: schoolName, registrator_email: registratorEmail })
        })
        ...
        .then(data => {
            if (data.success) {
                ...
                sessionStorage.removeItem('registratorEmail')
...
resendButton.addEventListener("click", () => {
    ...
    .then(data => {
            ...
            // Start the timer for 60 seconds only if OTP was sent successfully
            let timer = 60;
            ...
            // Notify the user of the failure without starting the timer
            showNotification("info", 'Failed to resend OTP. Please try again.');

            // Re-enable the button if OTP send failed
            resendButton.classList.remove('disabled');
            resendButton.innerHTML = 'Resend OTP verification code';
            canResend = true; // Allow resend immediately
        }
    })
    .catch(error => {
